...
for i=1:nEpochs
    ...
    elseif length(event2Latency)==1 && length(event1Latency)>1 % if there's more than one event1 in the epoch, get the last event1 BEFORE event2.
        closestPositive = find(event1Latency<event2Latency,1,'last');
        timeBetween(i) = event2Latency-event1Latency(closestPositive);
    elseif length(event1Latency)==1 || length(event2Latency)==1
