...
% Updated 7/29/11 by DJ - uses Numbers Struct instead of hard-coded
% constants
...
for i = 1: size(mainStruct.eeg.object_events)  % Run for each event in event array
    ...
    curType = floor(curObj/Numbers.ENTERS); % Determine what type of event it is (0 = saccade, 1 = object appearance, 2 = disappearance)
    
    if curType == 1  % If object appears
       appearedObjects = vertcat(appearedObjects,  [curTime, (curObj-Numbers.ENTERS)]);  % Store time/object in array
    end
    if curType == 2  % If object disappears
       disappearedObjects = vertcat(disappearedObjects,  [curTime, (curObj-Numbers.EXITS)]);  % Store time/object in another array
    end
end
...
for i = 1:y  % Loop for the each button press
    ...
    for j = 1:size(truncatedDisappearedObjects,1)
        for k = 1:size(truncatedAppearedObjects,1)
    ...
    if size(truncatedAppearedObjects,1) ~= 1  % If the total number of objects on the screen doesn't equal 1, insert NaN into object array
        ...
        for k = marker1:marker2  % Go through the original event vector from the two elements determined above. When saccade is found, break accordingly.
            if floor(mainStruct.eeg.object_events(k,2)/Numbers.ENTERS) == 0  % Element being looked at is a saccade
