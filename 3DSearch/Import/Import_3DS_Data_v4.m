function x = Import_3DS_Data_v4(subject,ascSession,eegSession,eegFiletype,pixelThresholds,timeLimits,useReplay)

% x = Import_3DS_Data_v4(subject,ascSession,eegSession,eegFiletype,pixelThresholds,timeLimits,useReplay)
%
...
if nargin<6 || isempty(useReplay)
    useReplay = 1; % for Unity <v7.1, object bounding boxes were found with 
                   % a Unity replay
end
...
if useReplay
    ...
    event_filename = eye_filename; % find bounding boxes in .asc file
end
switch eegFiletype
    case 'Biosemi'
    ...
    case 'Sensorium-2013'
        eeg_filename = sprintf('3DS-%d.%.3d.bh',subject,eegSession); % .dat EEG file
end
...
object_limits = get_objectlimits(event_filename,~useReplay);
...
switch eegFiletype
    ...
    case 'Sensorium-2013'
        ...
        sync_events = [iEvents', eeg_sync_data(iEvents)'];
        x.eeg.sync_events = sync_events(ismember(sync_events(:,2),[Numbers.START_RECORDING, Numbers.SYNC,0]),:);
