...
% Updated 9/26/14 by DJ - added heogtypes input.

if nargin<2 || isempty(offset_ms)
    offset_ms = 0;
end
if nargin<3 || isempty(heogtypes)
...
iEventsFSL = find(strcmp(heogtypes{1,1},eventType));
iEventsFEL = find(strcmp(heogtypes{1,2},eventType));
iEventsFSR = find(strcmp(heogtypes{2,1},eventType));
iEventsFER = find(strcmp(heogtypes{2,2},eventType));
