...
% Updated 8/6/10 by DJ (made time window inclusive, and made it absolute value)

% Handle inputs
if nargin==2
    t_window = [t_samples(1) t_samples(end)];
end

%% Find maxes of each trial
inWindow = (t_samples >= t_window(1) & t_samples <= t_window(2));
maxes = max(max(abs(data(:,inWindow,:)),[],2),[],1);
...
title(sprintf('Voltage cutoff exclusion histogram - event + [%d, %d]ms',round(t_window(1)),round(t_window(2))));
