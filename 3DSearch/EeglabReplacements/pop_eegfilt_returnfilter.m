% pop_eegfilt_returnfilter() - interactively filter EEG dataset data using eegfilt() and
% return the filter coefficients!
%
% Usage:
%   >> [EEGOUT, com, b] = pop_eegfilt( EEG, locutoff, hicutoff, filtorder, revfilt, usefft, plotfreqz););
%
% Graphical interface:
...
%   plotfreqz - [0|1] 1 plot the frequency response of the filter using
%                     freqz().  Default is 0.
%
% Outputs:
%   EEGOUT   - output dataset
%   com      - command executed
%   b        - coefficients of the filter chosen by this program (if fft
%              filter is used, b=0)
%
% Author: Arnaud Delorme, CNL / Salk Institute, 2001
%
% See also: pop_eegfilt, eegfilt(), eegfiltfft(), eeglab()
%
% Updated 8/31/10 by DJ - Copied pop_eegfilt, made b (the filter
%    coefficients) an output.
% Updated 7/26/11 by DJ - allow plotfreqz input, added to comments
% Updated 1/6/12 by DJ - fft filter returns b=0
...
function [EEG, com, b] = pop_eegfilt_returnfilter( EEG, locutoff, hicutoff, filtorder, revfilt, usefft, plotfreqz);
