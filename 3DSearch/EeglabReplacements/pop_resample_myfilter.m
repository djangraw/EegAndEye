% pop_resample_myfilter() - resample dataset with a given filter.
%
% Usage:
%   >> [OUTEEG] = pop_resample( INEEG, freq, filtercoeffs);
%
% Graphical interface:
...
%   filtercoeffs    - low-pass filter coefficients, as from pop_eeg_returnfilter
%
% Outputs:
...
% Updated 8/31/10 by DJ - Copied pop_resample, made filtercoeffs an input
% Updated 8/4/11 by DJ - fixed urevent bug: now reads 'for index1 = 1:length(EEG.urevent)'

function [EEG, command] = pop_resample_myfilter( EEG, freq, filtercoeffs); 
...
for index1 = 1:size(EEG.data,1)
    ...
    if index1 == 1
        ...
        for ind = 1:length(bounds)-1
            tmpres  = [ tmpres; myresample( double( sigtmp(bounds(ind):bounds(ind+1)-1,:)), p, q, usesigproc, filtercoeffs ) ];
        ...
        for ind = 1:length(bounds)-1
            tmpres(indices(ind):indices(ind+1)-1,:) = myresample( double( sigtmp(bounds(ind):bounds(ind+1)-1,:) ), p, q, usesigproc, filtercoeffs );
        end;
    end; 
...
% -----------------------------------
function tmpeeglab = myresample(data, pnts, new_pnts, usesigproc, filtercoeffs);
    
    if usesigproc
        tmpeeglab = resample(data, pnts, new_pnts, filtercoeffs);
        return;
    end;
