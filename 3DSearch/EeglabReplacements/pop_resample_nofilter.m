...
% Updated 1/6/11 by DJ - copied from pop_resample_myfilter, got rid of
%     filtercoeffs input, set last input to myresample to 0.

function [EEG, command] = pop_resample_nofilter( EEG, freq); 
...
for index1 = 1:size(EEG.data,1)
    ...
    if index1 == 1
        ...
        for ind = 1:length(bounds)-1
            tmpres  = [ tmpres; myresample( double( sigtmp(bounds(ind):bounds(ind+1)-1,:)), p, q, usesigproc, 0 ) ];
        ...
        for ind = 1:length(bounds)-1
            tmpres(indices(ind):indices(ind+1)-1,:) = myresample( double( sigtmp(bounds(ind):bounds(ind+1)-1,:) ), p, q, usesigproc, 0 );
        end;
    end; 
