function [y, w, v, fwdModel, y_level1] = RunHybridHdcaClassifier_LR(data, truth, trainingwindowlength, trainingwindowoffset, cvmode, level2data, fwdModelData)

% Run a hierarchical classifier with added "level 2" data.
%
% [y, w, v, fwdModel, y_level1] = RunHybridHdcaClassifier_LR(data, truth, trainingwindowlength,
...
% Updated 4/12/14 by DJ - switched to LR for level1 classification
% Updated 4/14/14 by DJ - made lambda=1000 for level1, 10 for level2.
%   De-mean y values before calculating fwd model.
...
for foldNum=1:nFolds       
    ...
    for jFold = 1:cv_fold.numFolds
        ...
        for iWin=1:nWindows
            ...
            % Find spatial weights using LR
            params.vinit=zeros(nElecs+1,1);
            [~,~,stats] = RunSingleLR(innerTrainingData(:,isInWin,:),squeeze(innerTrainingTruth(1,1,:)),params);
            % Extract spatial weights (ignore offset)
            wCell{foldNum}(:,iWin,jFold) = stats.wts(1:nElecs);
