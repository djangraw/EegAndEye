function [y, w, v, fwdModel, y_EEG] = Run2LevelClassifier_nested(data, truth, trainingwindowlength, trainingwindowoffset, cvmode, level2data, fwdModelData)

% Run a 2-step classifier similar to that used in LIINC RSVP experiments.
%
% [y, w, v, fwdModel, y_EEG] = Run2LevelClassifier_nested(data, truth, trainingwindowlength,
...
params.regularize=1;
% params.lambda = [repmat(nElecs,1,nWindows), ones(1,nWindowWeights-nWindows)];
params.lambda=1e1;
...
%             w(:,iWin,foldNum,jFold) = coeff(2).linear;
    ...
    % FOR NOW, MAKE EVAL AND TRAINING DATA THE SAME FOR ADD-ONS!
...
%     y0data = permute(yEvalData(foldTrainingTruth_trials==0,:),[2,3,1]);
%     y1data = permute(yEvalData(foldTrainingTruth_trials==1,:),[2,3,1]);
%     foldALLEEG = pop_editset(eeg_emptyset,'data',y0data);
%     foldALLEEG = eeg_checkset(foldALLEEG);
%     foldALLEEG(2) = pop_editset(eeg_emptyset,'data',y1data);
...
%     vCell{foldNum} = ALLEEGout(1).icaweights;
%     v(:,:,foldNum) = ALLEEGout(1).icaweights;
%     yEvalTrials = vCell{foldNum}*yEvalData';
