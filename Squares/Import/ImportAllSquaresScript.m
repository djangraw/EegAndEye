...
experiments = {'Squares','SquaresFix','SquaresFix3'};
...
for iExp = 1:numel(experiments)
    ...
    for iSubj = 1:numel(subjects)
        ...
        % Redo check
        if exist(sprintf('%s-%d%s.set',prefix,subject,combosuffix_interp),'file')~=0
            fprintf('Skipping %s-%d interpolation...\n',prefix,subject);
            continue;
        end
        
        % Remove outer electrodes
        RemoveElectrodes(sprintf('%s-%d%s.set',prefix,subject,combosuffix),sprintf('%s-%d%s.set',prefix,subject,combosuffix_interp),outerElecs);
        % Interpolate bad electrodes & overwrite
        InterpolateElectrodes(sprintf('%s-%d%s.set',prefix,subject,combosuffix_interp),sprintf('%s-%d%s.set',prefix,subject,combosuffix_interp),setdiff(badElecs,outerElecs));
