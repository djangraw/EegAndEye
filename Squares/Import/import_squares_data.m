function x = import_squares_data(subject, ascSession, eegSession, eegFiletype, pixel_threshold, drift_correct)

% Import data from a squares experiment
%
% x = import_squares_data(subject, ascSession, eegSession, pixel_threshold)
%
% INPUTS:
...
% the center of a square and the saccade endpoint that will still be 
% considered a saccade to that square.  (currently squares are 150 pixels
% apart).
...
% Updated 10/18/11 by DJ - completed!
% Updated 10/27/11 by DJ - made changes to match 3DSearch files
% Updated 10/31/11 by DJ - comments
% Updated 11/16/11 by DJ - new sync structure, new figures for each session
% Updated 12/28/11 by DJ - added button.trialnum field
% Updated 1/4/12 by DJ - added circle time field
% Updated 1/6/12 by DJ - fixed error if circle time doesn't exist
% Updated 2/2/12 by DJ - got rid of trial-by-trial drift correction
% Updated 3/2/12 by DJ - support for other target_colors, v1pt5 rxn times
% Updated 4/18/12 by DJ - made trial-by-trial drift correction an option
% Updated 7/2/12 by DJ - added recording_start_time
% Updated 7/23/12 by DJ - added rotation measure to datastruct
...
eye_filename = sprintf('sq_%d_%d.asc',subject,ascSession); % eyelink .edf file converted to text using Visual EDF2ASC.

switch eegFiletype
    case 'Biosemi'
        eeg_filename = sprintf('sq-%d-%d.bdf',subject,eegSession); % .bdf EEG file.
    case {'Sensorium-2008' 'Sensorium-2011' 'Sensorium-2011-1000Hz' 'Sensorium-2011-50Hz'}
        eeg_filename = sprintf('sq-%d.%.3d.b',subject,eegSession); % .dat EEG file
    case 'Sensorium-2013'
        eeg_filename = sprintf('sq-%d.%.3d.bh',subject,eegSession); % .dat EEG file
end
...
for i=1:numel(x.trial.type)
    ...
    for j=1:5
        if ismember(trialstring(j+1),'35')
            middle_color(i,j) = 'R';
        elseif ismember(trialstring(j+1),'16')
            middle_color(i,j) = 'G';
        elseif ismember(trialstring(j+1),'24')
            middle_color(i,j) = 'B';
        end
    end
end
    ...
    case 'Squares_v1pt5'
        for i=1:numel(x.trial.start_time)
            ...
            if ~isempty(iFirstButton) && i==numel(x.trial.start_time) || x.button.time(iFirstButton)<x.trial.start_time(i+1) % CHANGED FROM V1PT4
                button_trialnum(iFirstButton) = i;
                response_time(i) = x.button.time(iFirstButton);
                reaction_time(i) = response_time(i)-x.trial.circle_time(i); % CHANGED FROM V1PT4
                is_target_response(i) = (x.button.number(iFirstButton)==Constants.TARGET_BUTTON);        
            else
                response_time(i) = NaN;
                reaction_time(i) = NaN;
...
%% SAVE
disp('Saving data...')
save(sprintf('sq-%d-%d.mat',subject,ascSession),'x');
disp('...DONE!')
